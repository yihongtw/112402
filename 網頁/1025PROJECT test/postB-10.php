...
if (isset($_POST["action"]) && ($_POST["action"] == "add")) {
    ...
    // 根据文件名（filename）从外部数据表中检索bookid
    $filename = "images/book1-10.png"; // 请将此替换为实际的文件名
    $query_select_bookid = "SELECT bookid FROM ccs_image WHERE filename = ?";
    $stmt = $db_link->prepare($query_select_bookid);
    ...
    if ($bookid) {
        // 插入留言，包括用户名（如果已登录）
        $query_insert = "INSERT INTO totalbook (bookid, userid, username, boardsex, boardcontent, boardtime) VALUES (?, ?, ?, ?, ?, NOW())";
        $stmt = $db_link->prepare($query_insert);
        $stmt->bind_param("sssss", $bookid, $userid, $loggedInUsername, $boardsex, $boardcontent);
        $stmt->execute();
        $stmt->close();
        // 重新导向回到主页面
        header("Location: 股票心得1-10.php");
    } else {
        echo "未找到匹配的bookid。";
    }
}
...
<body>
    <nav class="home">
        <table>
            <tr>
                <td>
                    <a class="bd5" href="股票心得1-10.php"><img name="board_r1_c5" src="images/瀏覽留言.png" width="110" height="36" ,border="0" alt="瀏覽留言"></a>
                </td>
                <td>
                    <a class="bd7" href="post1-10.php"><img name="board_r1_c7" src="images/我要留言.png" width="110" height="36" ,border="0" alt="我要留言"></a>
                </td>
            </tr>
        </table>
    </nav>
    ...
    <form action="" method="post" name="formPost" id="formPost" onSubmit="return checkForm();">
        <table width="90%" border="0" align="center" cellpadding="4" cellspacing="0">
            <tr valign="top">
                <td class="book" width="80" align="center"><img src="images/book1-10.png"  width="200" height="200"><span class="heading"></span></td>
                <td>
